...
class MEmbed_redtube
{
	...
	public function get_videos() {
		...
		if ($html) {
			...
			if (preg_match_all('/video-listing(.*?)<\/ul>/', $html, $html_videos)) {
			...
			if ($matches[0]) {
				foreach ($matches[0] as $match) {
					...
					//Video ID
					if (preg_match('/<a href="\/(.*?)"/', $match, $matches_id)) {
					...
					if(preg_match('/<a href="(.*?)"/', $match, $matches_url)) {
						$video['url']   = "https://www.redtube.com".trim($matches_url[1]);
					} else {
					...
					//Duration
					if(preg_match('/video-duration(.*?)">(.*?)<\/span>/', $match, $matches_duration)) {
                        $video['duration'] = duration_to_seconds($matches_duration[2]);
                    } else {
					...
					//Categories
					if (preg_match_all('/category-list-item-name">(.*?)</', $html_video, $matches_cat)) {
					...
					//Tags
					if (preg_match_all('/tag-item-name">(.*?)</', $html_video, $matches_tags)) {
						$video['tags'] = implode(' ', str_replace(' ','-',$matches_tags[1]));
					...
					if (!strpos($html_video, 'class="embed-code"')) {
