...
class MEmbed_pornrabbit
{
	...
	public function get_videos() {
		...
		if ($html) {
			if (preg_match_all('/<div class="video">(.*?)<\/div><\/div>/', $html, $matches)) {
				foreach ($matches[0] as $match) {
					...
					//Video ID
					if (preg_match('/<a href="\/video\/(.*?)\//', $match, $matches_id)) {
					...
					//Embed Code					
					$video['embed'] = '<iframe src="https://www.pornrabbit.com/embed/' . $video['id'] . '" width="' . E_WIDTH . '" height="' . E_HEIGHT . '" style="overflow: hidden" frameborder="0" scrolling="no"></iframe>';					
					...
					if(preg_match('/<a href="(.*?)"/', $match, $matches_url)) {
						$video['url']   = "https://www.pornrabbit.com".trim($matches_url[1]);
					} else {
					...
					//Duration
					if(preg_match('/runtime: <b>(.*?)</', $match, $matches_duration)) {
					...
					//Thumbnails
					if (preg_match('/ethumbs:\[(.*?)\]/', $match, $matches_thumb)) {
                        $thumb_str = $matches_thumb[1];
						$thumb_str = explode(',', $thumb_str);
						if (isset($thumb_str[1])) {
							foreach ($thumb_str as $thumb_url) {
								$video['thumbs'][] = trim(str_replace("'", "", $thumb_url));
							}
						}
						else {
					...
					//Categories
					if(preg_match('/id="video-details"(.*?)<\/div>/', $html_video, $matches_category)) {
						if (preg_match_all('/<a href="\/videos\/(.*?)\/"><b>(.*?)<\/b>/', $matches_category[1], $matches_cat)) {
					...
					//Tags
					if (preg_match_all('/<a href="\/videos\/tags\/(.*?)">(.*?)<\/a>/', $html_video, $matches_tags)) {
					...
					if (!strpos($html_video, '<div class="textshare"><span>Embed')) {
