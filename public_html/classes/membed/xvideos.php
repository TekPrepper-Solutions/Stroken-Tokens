...
class MEmbed_xvideos
{
	...
	public function get_videos() {
		...
		if ($html) {
			if (preg_match_all('/<div id="video_(.*?)<\/span><\/p><\/div>/', $html, $matches)) {
				foreach ($matches[0] as $match) {
					...
					//Embed Code
					$video['embed'] = '<iframe src="https://flashservice.xvideos.com/embedframe/' . $video['id'] . '" frameborder=0 width=' . E_WIDTH . ' height=' . E_HEIGHT . ' scrolling=no style="background-color:#000"></iframe>';					
					...
					if(preg_match('/<a href="(.*?)"/', $match, $matches_url)) {
						$video['url'] = "https://www.xvideos.com" . trim($matches_url[1]);
					} else {
					...
					if(preg_match('/<span class="duration">(.*?)<\/span>/', $match, $matches_duration)) {
					...
					//Thumbnails
					if (preg_match('/<img src="(.*?)" data-src="(.*?)" data-idcdn="(.*?)" data-videoid="(.*?)" id="pic_(.*?)" \/>/', $match, $matches_thumb)) {
						$thumb_url       = $matches_thumb[2];
