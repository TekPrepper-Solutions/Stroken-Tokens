...
if ( isset($_POST['grab_video']) ) {
    ...
    if ( !$errors ) {
		...
		if ($tags == '') {
			$tags = get_tags($video['title']);
		}
        $tags                 = str_replace(',', '', $tags);
		$word_array           = preg_split('/[\s?:;,.]+/', $tags, -1, PREG_SPLIT_NO_EMPTY);
		$unique_word_array    = array_unique($word_array);
		$tags                 = implode(' ',$unique_word_array);
        $video['tags']        = $tags;
...
if ( isset($_POST['save_video']) ) {
    ...
    if ( !$errors ) {
        $sql        = "INSERT INTO video (UID, title, channel, vkey, keyword, description, type, addtime, adddate)
		...
		$info[] = 'The video is now downloading and convertig. You may leave/close this window.';
		$grabbing = TRUE;
    }
}
